...
import { OauthService, ExchangeAccessTokenResponse } from "@canva/connect-api-ts/ts";
...
export async function getAccessToken(token: string): Promise<string> {
    const storedToken = JSON.parse(token) as ExchangeAccessTokenResponse;
    // If the access token is not expiring in the next 10 minutes, we can keep using it
    const claims = jose.decodeJwt(storedToken.access_token);
    const refreshBufferSeconds = 60 * 10; // 10 minutes;
    if (claims.exp) {
...
export function getBasicAuthClient(): typeof client {
