...
import {
    ...
    getAuthorizationUrl, TOKEN_AUTH_COOKIE_NAME,
} from "../services/auth";
import * as jose from "jose";
// import { deleteToken, getToken, setToken } from "../database/queries";
import {getAccessToken, getBasicAuthClient} from "../services/client";
import { OauthService } from "@canva/connect-api-ts/ts";
...
router.get(endpoints.REDIRECT, async (req, res) => {
    console.log("globals.redirectUri:" ,globals.redirectUri)
    ...
    console.log("ini signedCookies:", req.signedCookies[OAUTH_STATE_COOKIE_NAME])
    console.log("ini cookies:", req.cookies[OAUTH_STATE_COOKIE_NAME])
    ...
    try {
        ...
        // instead of setToken to DB, we set token to auth
        res.cookie(
            TOKEN_AUTH_COOKIE_NAME, token, {
                httpOnly: true,
...
router.get(endpoints.REVOKE, async (req, res) => {
    ...
    // rather than getting token from user id, get token auth from cookie instead
    const token = await req.signedCookies[TOKEN_AUTH_COOKIE_NAME];
    res.clearCookie(AUTH_COOKIE_NAME);
    res.clearCookie(TOKEN_AUTH_COOKIE_NAME);
    if (!token) {
    ...
    try {
        ...
        // no need to delete token, as token cookie is deleted already
    }
